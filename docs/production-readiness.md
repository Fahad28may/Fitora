...
| Data leakage | NEEDS REVIEW | Only dashboard/weight/workout data sent to AI provider, confirmed by code review; photo recognition sends the image and a fixed prompt only, asserted by test; provider's own retention/training policy not yet independently verified |
...
| Output validation | PASS | Food parsing: Pydantic schema + one retry + 422 fallback. Coach: non-empty, length-capped. Photo recognition: schema has no scalar calorie field at all, so an exact-number answer is unrepresentable, and any range narrower than 20% of its midpoint is widened before it reaches the user |
...
| Device record ids (`external_id`) | NEEDS REVIEW | Dedup correctness depends on the native implementation using HealthKit's UUID / Health Connect's record id rather than synthesising one — the single constraint the interface cannot enforce. Two of its three failure modes are now caught at runtime: colliding ids within one read are dropped instead of overwriting each other server-side, and ids that change between overlap re-reads are detected and reported to the user as a fault in Fitora (`dedupeById` / `idsLookUnstable`, 7 tests). The third — an id that is stable but not the device's — is undetectable from the client and needs checks 5 and 6 of the `health-integrations.md` runbook on real hardware. Stays NEEDS REVIEW until those pass |
